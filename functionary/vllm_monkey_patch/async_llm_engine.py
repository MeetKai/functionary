...
class _AsyncLLMEngine(LLMEngine):
    ...
    def check_end_of_sampling(self, suffix, prompt_template_version, request_id):
        ...
        """
        # Return True if the suffix is one of the function names
        if any(
            [
        ...
        # Return True if the latest_param_str is one of the parameter names
            ...
            for i in range(len(curr_params_str) - 1, -1, -1):
                try:
                    ...
                    if any(
                        [
                            latest_param_str.lstrip('"') == parameter_name
                            for parameter_name in parameter_options
                        ]
                    ):
                        return True
    ...
    async def step_async(self) -> List[RequestOutput]:
        ...
        for i in range(len(output)):
            ...
            # This is to check if the complete function/parameter name is generated
            # and replace the intended model token to the token right after the name
            # (e.g.: get_current_weather_ -> get_current_weather\n). This stops the
            # model from hallucinating a function name that has the intended function
