    ...
    def update_grammar_sampling_gen_state(
        ...
        if gen_state["stage"] == "pre-function":
            ...
            if gen_state["curr_text"].endswith(
                tokenizer.decode([self.get_stopping_token(stage="function")])
            ) or (
            ...
            # Loop through curr_params_str from the back char-by-char and try to see if
            # the substring can be converted to json. The remaining substring after that
            # will be the current parameter being generated by the model at this current step.
            for i in range(len(curr_params_str) - 1, -1, -1):
                try:
                    # Try to convert the substring left of i to json
                    _ = json.loads("{" + curr_params_str[:i] + "}")
                    # Extract the latest parameter being generated (right of i)
