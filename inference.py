...
def prepare_message_for_inference(message):
...
def prepare_messages_for_inference(messages, functions=None):
    all_messages = []
    ...
    all_input_ids = [prepare_message_for_inference(msg) for msg in messages]
    return torch.cat(all_input_ids, dim=-1)


def generate(messages, functions=None, temperature=0.7, max_new_tokens=256):
    inputs = prepare_messages_for_inference(messages, functions)
    generate_ids = model.generate(inputs, max_new_tokens=max_new_tokens, temperature=temperature)
    return tokenizer.batch_decode(generate_ids[:, inputs.shape[1]:], skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]


if __name__ == "__main__":
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    model = AutoModelForCausalLM.from_pretrained("musabgultekin/functionary-7b-v0.2", low_cpu_mem_usage=True, torch_dtype=torch.float16).to(device)
