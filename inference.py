...
def prepare_messages_for_inference(tokenizer, messages, functions=None):
    ...
    # print(all_messages)
    all_input_ids = [
        prepare_message_for_inference(tokenizer, msg) for msg in all_messages
    ]
    return torch.cat(all_input_ids, dim=-1)


def generate(
    model, tokenizer, messages, functions=None, temperature=0.7, max_new_tokens=256
):
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "musabgultekin/functionary-7b-v0.2", use_fast=False
    )
    ...
    out = generate(
        ...
        messages=[
            ...
            {
                "role": "assistant",
                "content": "The current weather in Istanbul is 32 degrees Celsius.",
            },
